...
from config_classes.dataclasses import ModelConfig
from mmd import mmd2
from model.blocks import block, mid_blocks
from model.model_utils import index_by_s, grad_reverse
...
class AE(LightningModule):
    def __init__(self, cfg: ModelConfig, num_s: int):
        super().__init__()
        self.enc = Encoder(
            in_size=1,
    ...
    def training_step(self, batch, batch_idx):
        ...
        z, s_pred, recons = self(x)
        recon_loss = l1_loss(index_by_s(recons, s), x, reduction="mean")
        adv_loss = mmd2(z[s == 0], z[s == 1], kernel="rbf") + binary_cross_entropy_with_logits(
            s_pred.squeeze(-1), s, reduction="mean"
        )
        loss = self.recon_weight * recon_loss + adv_loss

        with no_grad():
            cf_z, _, cf_recons = self(cf_x)
    ...
    def test_epoch_end(self, output_results) -> None:
