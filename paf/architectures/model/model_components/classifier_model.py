...
class Clf(CommonModel):
    ...
    def training_step(self, batch: Batch | CfBatch | TernarySample, *_: Any) -> Tensor:
        ...
        s_s0y0 = batch.x.new_zeros((x_s0y0.shape[1], 1))
        s_s0y1 = batch.x.new_zeros((x_s0y0.shape[1], 1))
        assert len(s_s0y0) == len(s_s0y1)

        y_s0y0 = batch.x.new_zeros((x_s0y0.shape[1], 1))
        y_s0y1 = batch.x.new_ones((x_s0y1.shape[1], 1))
        ...
        s_s1y0 = batch.x.new_ones((x_s0y1.shape[1], 1))
        s_s1y1 = batch.x.new_ones((x_s0y1.shape[1], 1))
        y_s1y0 = batch.x.new_zeros((x_s0y0.shape[1], 1))
        y_s1y1 = batch.x.new_ones((x_s0y1.shape[1], 1))
